...
public class Clock2Dao {
    ...
    public Clock2Dao(Context context) {
        mDbHelper = new MyDatabaseHelper(context,"Data.db", null, 2);
    }

    public Clock2Dao open() {
    ...
    public long create(Tomato tomato) {
        ...
        long id = db.insert("Clock2", null, values);
        close();
        return id;
    }
    ...
    public List<Tomato> getDbAllTomato() {
        ...
        Cursor cursor=db.rawQuery("SELECT * FROM Clock2", null);
        while(cursor.moveToNext()) {
...
//            data.setLongBreak(cursor.getInt(cursor.getColumnIndex("longBreak")));
        ...
        cursor.close();
        close();
        Log.i("Clock2Dao", "查询到本地的番茄任务个数：" + tomatoList.size());
        return tomatoList;
    }
...
//        }
    ...
    public void clearAll() {
        open();
        db.execSQL("delete from Clock2");
        db.execSQL("update sqlite_sequence set seq = 0 where name = 'Clock2' ");
        close();
    }
